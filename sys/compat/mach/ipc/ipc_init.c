...
ipc_bootstrap_sysinit(void *arg __unused)
{
	...
	if (ipc_space_create_special(&ipc_space_kernel) != KERN_SUCCESS)
		panic("%s: failed to create kernel IPC space", __func__);


	if (ipc_space_create_special(&ipc_space_reply) != KERN_SUCCESS)
		panic("%s: failed to create reply IPC space", __func__);
